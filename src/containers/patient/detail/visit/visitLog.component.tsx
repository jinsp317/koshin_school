...
import { PatientModel, VisitModel } from "@src/core/model";
...
interface ComponentProps {
  ...
  data: VisitModel[];
  onItemSelect: (item: VisitModel) => void;
...
class MyComponent extends React.Component<Props, State> {
  ...
  private onItemPress = (item: VisitModel) => {
      ...
      <View style={commonStyles.toolbarContainer}>
        <View style={{ flexDirection: "row" }}>
          <Text style={commonStyles.toolbarText}>
            {this.props.seedInfo && `${this.props.seedInfo.name}  ${this.props.seedInfo.mobile}`}
          </Text>
        </View>
      </View>
    );
  ...
  private renderHeader = () => {
    ...
    return (
      <View style={themedStyle.header}>
        {GLOBAL.HEADERITEMS_VISITS.map((item, idx) => {
          const flexValue = 1;
          const colW = idx == 1 ? 120 : idx === 5 ? 200 : 80;
          return (
  ...
  private renderItem = (item, index, separators) => {
    ...
    return (
      ...
      >
        <ListItem data={item} onItemPress={this.onItemPress} onShowImage={this.props.onShowImage} />
      </TouchableHighlight>
    );
  };
...
export const VisitLog = withStyles(MyComponent, (theme: ThemeType) => ({
  container: {
    flex: 1
  },
  contentContainer: {
