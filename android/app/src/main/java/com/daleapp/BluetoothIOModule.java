...
interface IConnection {
  void bytesReceived(String response);
  void signalConnect();
...
public class BluetoothIOModule extends ReactContextBaseJavaModule implements IConnection {
    ...
    final BroadcastReceiver mReceiver = new BroadcastReceiver() {
      ...
      public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
          ...
          sendEvent(Const.EVENT_ON_BLUETOOTH_STATE_CHANGE, params);
      ...
      // When discovery finds a device
        ...
        params.putInt("bondState", device.getBondState());
        sendEvent(Const.EVENT_ON_BLUETOOTH_DISCOVERY_FOUND, params);
        ...
        sendEvent(Const.EVENT_ON_BLUETOOTH_DISCOVERY_STOP);
  ...
  public Map<String, Object> getConstants() {
    ...
    constants.put("EventOnDataRx", Const.EVENT_ON_DATA_RX);
    constants.put("EventOnStateChange", Const.EVENT_ON_STATE_CHANGE);
    constants.put("EventOnBluetoothStateChange", Const.EVENT_ON_BLUETOOTH_STATE_CHANGE);
    constants.put("onBluetoothDiscoveryStart", Const.EVENT_ON_BLUETOOTH_DISCOVERY_START);
    return constants;
  }
  ...
  private void emitOnDataRx(String base64Content) {
    ...
    Log.d(TAG, "data = " + base64Content);
    sendEvent(Const.EVENT_ON_DATA_RX, params);
  }
  ...
  public void getDeviceList(String deviceNameFilter, Promise promise) {
    ...
    try {
      ...
      for(BluetoothDevice bluetoothDevice: deviceSet){
        ...
        bluetoothDeviceMap.putInt("bondState", bluetoothDevice.getBondState());
  ...
  public void discoveryStart() {
    ...
    sendEvent(Const.EVENT_ON_BLUETOOTH_DISCOVERY_START);
  }
  ...
  public void writeString(String data, Promise promise) {
    try {
      ...
      Log.d(TAG, String.format("writeString[%d]={%s}", sendData.length(), sendData));
      // Debug event emitOnDataRx
      //emitOnDataRx(data);
     
      byte[] byteArray = sendData.getBytes(StandardCharsets.UTF_8);
      mChatService.write(byteArray);

      promise.resolve(mChatService.getState());
    } catch (Exception ex) {
      ex.printStackTrace();
      promise.reject(ex);
    }
  ...
  public void bytesReceived(String response) {
    Log.d(TAG, String.format("IConnection: bytesReceived %d bytes", response.length()));
  ...
  public void signalStateChanged(int state) {
    ...
    params.putInt("state", state);
    params.putString("name", stateName);
    sendEvent(Const.EVENT_ON_STATE_CHANGE, params);
  }
