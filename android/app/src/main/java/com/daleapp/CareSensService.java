...
class CareSensService {
    ...
    private CareSensModule mModule;
     ...
     */
    CareSensService(CareSensModule module) {
    ...
    private void connectionFailed(BluetoothDevice device) {
        Log.d(TAG, "called connectionFailed 002");
        mModule.onConnectionFailed("Unable to connect to device", device); // Send a failure message with device
        CareSensService.this.stop(device.getAddress()); // Start the service over to restart listening mode
    }
    ...
    private class ConnectThread extends Thread {
        ...
        public void run() {
            ...
            synchronized (CareSensService.this) {
