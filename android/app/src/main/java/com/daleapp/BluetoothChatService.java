...
public class BluetoothChatService {
  ...
  */
  ...
  device, final String socketType) {
    Log.d(TAG, "Bluetooth connect Socket Type:" + socketType);
  ...
  */
  private void connectionFailed() {
    ...
    Log.d(TAG, String.format("Unable to connect device"));
  ...
  */
  private void connectionLost() {
    ...
    // msg.setData(bundle);
    // mHandler.sendMessage(msg);
    Log.d(TAG, String.format("Device connection was lost"));
    ...
    public void run() {
      ...
      while (mState != STATE_CONNECTED) {
        try {
          ...
          Log.e(TAG, "Socket Type: -" + mState + "- accept() failed", e);
          mState = STATE_NONE;
          cancel();
          break;
        }
  ...
  private class ConnectThread extends Thread {
    ...
    public void run() {
      ...
      try {
        ...
        Log.e(TAG, "Connect IOException " + e.getMessage(), e);
    ...
    public void run() {
      ...
      while (mState == STATE_CONNECTED || mState == STATE_CONNECTING) {
        try {
          ...
          avaliable = mmInStream.available();
          if(avaliable > 0)
          {
            bytes = mmInStream.read(buffer);
            String dbgOutput = new String(buffer, 0, bytes, "UTF-8");
            Log.e(TAG,  String.format("ConnectedThread Rx[%d]={%s}",bytes, dbgOutput));
            if(!bEnded){
              if(dbgOutput.startsWith("{-*-}")){
                dbgOutput = dbgOutput.substring(5);
                Log.e(TAG,  String.format("First {%s}", dbgOutput));   
                strBuffer = new StringBuilder(dbgOutput);             
                // strBuffer.append(dbgOutput);
                bEnded = true;
              }
              else {
                Log.e(TAG,  String.format("ConnectedThread Rx[%d]={%s}",bytes, dbgOutput));
                mConnectionSubscriber.bytesReceived(dbgOutput);                
              }
            } 
            else {
              if(dbgOutput.endsWith("{===}")){
                dbgOutput = dbgOutput.replaceAll("\\{\\=\\=\\=\\}", "");
                strBuffer.append(dbgOutput);
                Log.e(TAG,  String.format("Completed {%s}", strBuffer.toString()));
                bEnded = false;
                mConnectionSubscriber.bytesReceived(strBuffer.toString());
              } 
              else {
                if(dbgOutput.contains("{===}") && dbgOutput.indexOf("{===}") > 0){
                  int pos = dbgOutput.indexOf("{===}") -1 ;
                  strBuffer.append(dbgOutput.substring(0, pos));
                  Log.e(TAG,  String.format("Completed {%s}", strBuffer.toString()));
                  bEnded = false;
                  mConnectionSubscriber.bytesReceived(strBuffer.toString());
                  // String rem = dbgOutput.substring(pos+6);
                  // if(rem.startsWith("{-*-}"))
                  // {
                  //   rem = rem.substring(5);
                  // }
                  // strBuffer = new StringBuilder(rem);  
                } 
                else {
                  strBuffer.append(dbgOutput);
                  Log.e(TAG,  String.format("continue {%s}", outString));
                 }                
              }
            }       
          }
