...
export default class UltimateListView extends Component {
    ...
    static propTypes = {
        ...
        waitingSpinnerTextStyle: PropTypes.any,
    ...
    refreshAdvanced = () => {
        this.onRefresh();
        if (this._flatList._listRef._scrollRef && this._flatList._listRef._scrollRef.startRefresh){
            this._flatList._listRef._scrollRef.startRefresh();
        }
    }
    ...
    postRefresh = (rows = [], pageLimit) => {
        if (this.mounted) {
            let isAllLoaded;
            //如果allLoadedProof不为空, 那么依据allLoadedProof的返回值来确定是否已全部加载
            const {allLoadedProof} = this.props;
            if (allLoadedProof) {
                isAllLoaded = allLoadedProof();
            }else {
                isAllLoaded = rows.length < pageLimit;
            }
            ...
            if (this.props.autoPagination) {
                ...
                return (
                    <View style={styles.paginationView}>
                        ...
                        />
                        <Text
                            style={[styles.paginationViewText, {marginLeft: 10}]}
    ...
    render() {
        const {numColumns, renderScrollComponent} = this.props;
        return (
            <FlatList
                renderScrollComponent={renderScrollComponent ? renderScrollComponent : this.renderScrollComponent}
                key={numColumns}
